...
render_runtime = function(combined_modules){
  ...
  if (yuicompress_enabled) {
    yuicompress_archive(combined_source, function(combined_source){
...
yuicompress_archive = function(combined_source, callback){
  var data = ""
  ,   cmd
  ,   tmp_file
  ;
  
  console.log("Compressing with YUI Compressor");
  
  tmp_file = '/tmp/yui-compress-'+Math.random()+'.js';
  fs.writeFileSync(tmp_file, combined_source);
  
  cmd = spawn('yuicompressor', [tmp_file]);
  cmd.stdout.on('data', function(chunk){ data += chunk.toString(); });
  cmd.stderr.on('data', function(chunk){ console.log(chunk.toString().trim()); });
  cmd.stdin.end();
...
watch_tree = function(wpath, clb){
  ...
  fs.watchFile(wpath, {interval: 100}, function(curr, prev) { clb(wpath, curr, prev) });
