...
cli.command('build', "Build the archive", function(cmd){
  ...
  cmd.start(function(values){
    ...
    , output:    values.output || path.dirname(package_json)+'/package.js'
    , devbuild:  values.dev
    };
...
cli.command('generate', "Generate a some files", function(cmd){
  cmd.command("website", "Generate a website project", function(ccmd){
  ...
  cmd.start();
  // cmd.start(function(values){
  //   require('../generator')(values.positional);
  // });
});
