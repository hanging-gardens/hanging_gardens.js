var anim      = require('animator')
,   SM2       = require('soundmanager')
...
function ThreeSixtyPlayer() {
  var self = this;
  var pl = this;
  var sm = SM2.soundManager; // SM2.soundManager instance
  ...
  this.config = {
    ...
    loadRingColor: '#eaeaea', // how much has loaded
    playRingColor: '#d8d8d8', // how much has played
    backgroundRingColor: '#eaeaea', // color shown underneath load + play ("not yet loaded" color)

    // optional segment/annotation (metadata) stuff..
    segmentRingColor: 'rgba(255,255,255,0.33)', // metadata/annotation (segment) colors
    segmentRingColorAlt: 'rgba(0,0,0,0.1)',
    loadRingColorMetadata: '#dfdfdf', // "annotations" load color
    playRingColorMetadata: 'rgba(96,160,224,0.99)', // how much has played when metadata is present
    playRingColorMetadata: 'rgba(128,192,256,0.9)', // how much has played when metadata is present

    circleDiameter: 20, // set dynamically according to values from CSS
    circleRadius: 10,
    imageRoot: '', // image path to prepend for transparent .GIF - eg. /images/
    animDuration: 500,
    animTransition: anim.Animator.tx.bouncy, // http://www.berniecode.com/writing/animator.html
      ...
      thisSound._360data = {
        ...
        oAnimProgress: function(nProgress) {
          ...
          if (self.config.scaleFont && self.config.fontSizeMax != null) {
            thisSound._360data.oTiming.style.fontSize = parseInt(Math.max(1,self.config.fontSizeMax*nProgress), 10)+'px';
  ...
  this.fanOut = function(oSound) {
     ...
     SM2.soundManager._writeDebug('fanOut: '+thisSound.sID+': '+thisSound._360data.oLink.href);
  ...
  this.fanIn = function(oSound) {
     ...
     SM2.soundManager._writeDebug('fanIn: '+thisSound.sID+': '+thisSound._360data.oLink.href);
    // massive hack
  ...
  this.stopSound = function(oSound) {
    SM2.soundManager._writeDebug('stopSound: '+oSound.sID);
    SM2.soundManager.stop(oSound.sID);
    SM2.soundManager.unload(oSound.sID);
  };
...
fullCircle = (isOpera||isChrome?359.9:360); // I dunno what Opera doesn't like about this.
...
this.updatePlaying = function() {
  ...
  var timeNow = (self.config.showHMSTime?self.getTime(this.position,true):parseInt(this.position/1000, 10));
  ...
  this.updateWaveform = function(oSound) {
    ...
    */
    ...
    var offX = 0;
    var offY = parseInt(self.config.circleDiameter/2, 10);
  ...
  this.updateWaveformOld = function(oSound) {
    ...
    var oCanvas = oSound._360data.oCanvas.getContext('2d');
    var offX = 0;
    var offY = parseInt(self.config.circleDiameter*2/3, 10);
  ...
  this.init = function() {
    ...
    self.config.circleDiameter = parseInt(oFakeUIBox.offsetWidth, 10);
    self.config.circleRadius = parseInt(self.config.circleDiameter/2, 10);
    var oTiming = self.getElementsByClassName('sm2-timing','div',oTemp)[0];
    self.config.fontSizeMax = parseInt(self.getStyle(oTiming,'font-size'), 10);
    if (isNaN(self.config.fontSizeMax)) {
      // getStyle() etc. didn't work.
      self.config.fontSizeMax = null;
    }
    // SM2.soundManager._writeDebug('diameter, font size: '+self.config.circleDiameter+','+self.config.fontSizeMax);
    ...
    for (i=0,j=oLinks.length; i<j; i++) {
      if (sm.canPlayLink(oLinks[i]) && !self.hasClass(oLinks[i],self.excludeClass)) {
        ...
        if (isIE && typeof G_vmlCanvasManager != 'undefined') { // IE only
          ...
          var oID = 'sm2_canvas_'+parseInt(Math.random()*1048576, 10);
          o2.id = oID;
...
ThreeSixtyPlayer.prototype.VUMeter = function(oParent) {
  ...
  this.updateVU = function(oSound) {
    if (SM2.soundManager.flashVersion >= 9 && self.config.useFavIcon && self.config.usePeakData) {
...
ThreeSixtyPlayer.prototype.Metadata = function(oSound, oParent) {
  ...
  this.strToTime = function(sTime) {
    ...
    for (var i=segments.length; i--;) {
      seconds += parseInt(segments[i], 10)*Math.pow(60,segments.length-1-i,10); // hours, minutes
    }
    return seconds;
  };
...
if (navigator.userAgent.match(/webkit/i) && navigator.userAgent.match(/mobile/i)) {
  // iPad, iPhone etc.
  SM2.soundManager.useHTML5Audio = true;
}

SM2.soundManager.debugMode = (window.location.href.match(/debug=1/i)); // disable or enable debug output
SM2.soundManager.consoleOnly = true;
SM2.soundManager.flashVersion = 9;
SM2.soundManager.useHighPerformance = true;
SM2.soundManager.useFlashBlock = true;

// SM2.soundManager.useFastPolling = true; // for more aggressive, faster UI updates (higher CPU use)

// FPS data, testing/debug only
if (SM2.soundManager.debugMode) {
  var t = window.setInterval(function(){
    if (threeSixtyPlayer && threeSixtyPlayer.lastSound && threeSixtyPlayer.lastSound._360data.fps && typeof window.isHome === 'undefined') {
      SM2.soundManager._writeDebug('fps: ~'+threeSixtyPlayer.lastSound._360data.fps);
...
if (threeSixtyPlayer.config.useWaveformData) {
  SM2.soundManager.flash9Options.useWaveformData = true;
}
if (threeSixtyPlayer.config.useEQData) {
  SM2.soundManager.flash9Options.useEQData = true;
}
if (threeSixtyPlayer.config.usePeakData) {
  SM2.soundManager.flash9Options.usePeakData = true;
}

SM2.soundManager.onready(function(){
  if (SM2.soundManager.supported()) {
    // SM2.soundManager.createSound() etc. may now be called
    threeSixtyPlayer.init();
  }
});

exports.config = threeSixtyPlayer.config;
